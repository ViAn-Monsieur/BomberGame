...
//         public bool CheckSoloWin(out int winnerId)
//         {
...
//         public bool CheckTeamWin(out int winningTeam)
//         {
//             var aliveTeams = Players.Values
//                 .Where(p => p.IsAlive)
//                 .Select(p => p.TeamId)
//                 .Distinct()
//                 .ToList();
...
namespace BomberServer.Core
{
    public class Match
    {
        ...
        private void Explode(Bomb bomb)
        {
            ...
            var explosion = Explosion.Create(Map, bomb.X, bomb.Y, bomb.Power, out _);
            Explosions.Add(explosion);
